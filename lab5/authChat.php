 ...
 */
$comet = $link = mysqli_connect("app.comet-server.ru", "15", "lPXBFPqNg3f661JcegBY0N0dPXqUBdHXqj2cHf04PZgLHxT6z55e20ozojvMRvB8", "CometQL_v1");
...
if( isset($_GET["exit"]))
{   
    echo "Вы покинули php чат. <a href='http://localhost:8080/test/lab5/authChat.php'> Перейти к форме авторизации в чате</a>";
...
{ 
    ...
    if(  $users[$_GET["login"]]["pass"] !== $_GET["pass"] )
    {
        ...
        header("Location: http://localhost:8080/test/lab5/authChat.php");
        exit;
    }
    ...
    mysqli_query($comet, "INSERT INTO users_auth (id, hash)VALUES (1, '".mysqli_real_escape_string($comet, $userCometHash)."')"); 
    ...
    echo "Авторизация прошла успешно.<a href='http://localhost:8080/test/lab5/authChat.php'>Перейти к чату</a>";
    exit;
}
...
<!DOCTYPE HTML>
<head>
    <title>Типо почты только только как чат</title>
